...
const popupFormProfileSubmit = new PopupWithForm({
	...
	submitForm: (data) => {
		popupFormProfileSubmit.renderLoading(true);
		api.editInfoProfile(data)
			.then(res => {
			...
			.finally(() => {
				popupFormProfileSubmit.renderLoading(false);
			});
	}
})
...
const popupFormCardSubmit = new PopupWithForm({
	...
	submitForm: (data) => {
		popupFormCardSubmit.renderLoading(true);
		api.addCard(data)
			.then(res => {
			...
			.finally(() => {
				popupFormCardSubmit.renderLoading(false);
			});
	}
})
...
const handleAddLike = (cardId, card) => {
	api.addLikeCard(cardId)
		.then(res => {
			...
			card._countLikes.textContent = res.likes.length;
		})
		.catch((err) => {
			console.log(err);
		});
